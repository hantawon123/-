...
// Servo range
#define _DUTY_MIN 1300    //[3148]  서보의 가동 최소 각도
#define _DUTY_NEU 1450     //[3150] servo neutral position (90 degree)
#define _DUTY_MAX 1600                // [3169] 서보의 최대 가동 각도

// Servo speed control
#define _SERVO_ANGLE 30   //[3159] 서보의 각도(30º) 
//[3150] 레일플레이트가 사용자가 원하는 가동범위를 움직일때, 이를 움직이게 하는 서보모터의 가동범위
#define _SERVO_SPEED 75             //[3147]  서보 속도를 30으로 설정
...
// move servo to neutral position
  myservo.writeMicroseconds(1500);// [3147] 서보를 중간으로 이동
  duty_curr = 1500;
...
////////////////////
  ...
  // get a distance reading from the distance sensor
      ...
      last_dist = dist_filtered;
      if(dist_filtered > last_dist + 10 || dist_filtered < last_dist - 10)
        dist_filtered = last_dist;
        
      if(dist_filtered < b)
        dist_cali = 71+179.0/(b-a)*(dist_filtered-a);
      else
        dist_cali = 255+245.0/(c-b)*(dist_filtered-b);
